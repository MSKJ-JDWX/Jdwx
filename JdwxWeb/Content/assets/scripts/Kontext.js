window.kontext = function (container) {
    ...
    function show(target, direction) {
        ...
        if (layers[target] && !layers[target].classList.contains('x-show')) {

            layers.forEach(function (el, i) {
                el.classList.remove('left', 'right');
                el.classList.add(direction);
                if (el.classList.contains('x-show')) {
            ...
            layers[target].classList.add('x-show');
     ...
     *
     * param of [optional] layer DOM element which index is to be returned
     */
    function getIndex(of) {
        ...
        layers.forEach(function (layer, i) {
            if ((of && of == layer) || (!of && layer.classList.contains('x-show'))) {
                index = i;
                return;
            }
        });
